...
public class ConfigValidator {
    ...
    private static final int MIN_FOOD_STATIC_VALUE = 0;
    private static final int MAX_FOOD_STATIC_VALUE = 100;
    ...
    public void validate() {
        ...
        validateField(config.getFoodStaticValue(), MIN_FOOD_STATIC_VALUE, MAX_FOOD_STATIC_VALUE, "food.static");
        ...
        if (Double.compare(config.getProbabilityForFood(), MIN_DEAD_SNAKE_TO_FOOD_PROBABILITY) < 0
                || Double.compare(config.getProbabilityForFood(), MAX_DEAD_SNAKE_TO_FOOD_PROBABILITY) > 0) {
            throw new IllegalStateException(
                    String.format("%s=%f not from valid interval=[%f, %f]",
                            "deadSnake.to.food.probability",
                            config.getProbabilityForFood(),
