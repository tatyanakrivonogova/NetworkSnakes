...
public final class ConfigReader {
    ...
    public static Config readConfig(String configPath) {
        try (InputStream configStream = ConfigReader.class.getClassLoader().getResourceAsStream(configPath)) {
            Properties properties = new Properties();
            properties.load(configStream);
            return parseConfig(properties);
        } catch (IOException e) {
            logger.error("Can't open config file=" + configPath, e);
            throw new IllegalStateException("Can't open file = " + configPath, e);
        }
    }

    private static Config parseConfig(Properties properties) {
        Config.Builder builder = Config.Builder.config();
        readDoubleProperty(properties, "food.deadProbability")
